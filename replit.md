...
**Deployments surface — requires `ORBIT_GITHUB_TOKEN` GitHub secret (OrbitProduction env):** The deployments page fetches GitHub Actions run history via the GitHub API. The token must be a fine-grained PAT with Actions: Read-only on the tracked repos (`Orbit-Admin-Ctr`, `GrailBabe`). Named `ORBIT_GITHUB_TOKEN` (not `GITHUB_TOKEN`) in the OrbitProduction environment secrets to avoid collision with the built-in `GITHUB_TOKEN` that Actions uses for GHCR login. The deploy workflow passes it to the Container App as the env var `GITHUB_TOKEN`.
