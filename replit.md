...
- **Identity for Azure data:** the API authenticates to Azure (Resource Graph, Monitor, Cost Management, Microsoft Graph, Key Vault, App Configuration) via the user-assigned managed identity `id-sharedplatform-prod` — use `DefaultAzureCredential`, **no client secret**. (Integration code deferred; dashboard data is still mock.)
...
- `AZURE_MANAGED_IDENTITY_CLIENT_ID` — client ID of `id-sharedplatform-prod` (Azure Portal → Managed Identities → id-sharedplatform-prod → Overview → Client ID); tells DefaultAzureCredential which user-assigned identity to use
