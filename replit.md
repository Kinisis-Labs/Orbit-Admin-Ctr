...
- `artifacts/api-server/src/routes/orbit.ts` — All mock data (apps, telemetry, cost, alerts, revenue); the `APPS` inventory with the `userAuth: "clerk" | "entra" | "none"` flag + `clerkApps()`, the optional `androidPackage` flag + `playApps()` (apps tracked in Google Play), and the optional `group` field that drives the scope-selector grouping (e.g. the "Platform" section). Launch inventory: GrailBabe (prod, Clerk) and **Business Ops** (`id: "kinisis-labs"`, covers Orbit admin center + kinisislabs.com, `userAuth: "entra"`, grouped under "Platform", resource group `rg-kinisislabs-platform-shared-prod-eus2`, subscription `sub-sharedplatform-prod` — billing scope is full-subscription so all platform costs are captured together). Orbit was previously a separate APPS entry; its Azure resources and costs are now consolidated under Business Ops. GrailBabe-dev has been removed entirely.
...
- `artifacts/api-server/src/lib/thirdPartyUsage.ts` — Third-party AI API spend for the "API usage (MTD)" tile and "Third-party API costs" panel on the cost page. Config-gated: activates when `OPENAI_API_KEY__<APPID>` and/or `REPLICATE_API_TOKEN__<APPID>` are set; falls back to deterministic placeholder data otherwise. OpenAI: queries `/v1/organization/costs` + `/v1/organization/usage/completions` (MTD). Replicate: queries `/v1/billing/usage` (MTD). Env vars (set on Container App for GrailBabe): `OPENAI_API_KEY__GRAILBABE`, `REPLICATE_API_TOKEN__GRAILBABE`. Global fallbacks (no app suffix) also work if a single key covers all apps.
...
- **Identity for Azure data:** the API authenticates to Azure (Resource Graph, Monitor, Cost Management, Microsoft Graph, Key Vault, App Configuration) via the user-assigned managed identity `id-platformshared-prod` — use `DefaultAzureCredential`, **no client secret**. (Integration code deferred; dashboard data is still mock.)
- **Also provisioned:** App Configuration `appcs-orbit-prod-eus2` (`APP_CONFIGURATION_ENDPOINT`) and Application Insights `appi-oribit-prod` (resource name is misspelled "oribit"; `APPLICATIONINSIGHTS_CONNECTION_STRING`) — App Insights SDK wiring not built yet. App Configuration is wired: set these keys in the store to tune background scheduler cadences live without a redeploy (see `artifacts/api-server/src/lib/appConfig.ts`): `COST_REFRESH_INTERVAL_MINUTES`, `ALERT_CHECK_INTERVAL_MINUTES`, `ALERT_COOLDOWN_HOURS`, `ANOMALY_DISMISSAL_CLEANUP_HOURS`, `ANOMALY_DISMISSAL_RETENTION_DAYS`. A GitHub-OIDC federated managed identity (`…rg-orbit-prod-eus2Oidc`) also exists for the deploy workflow.
- **Prod resource names (confirmed from Azure portal, June 2026):** SWA `swa-orbit-prod`, Container App `ca-orbit-prod-v2` (in `sub-sharedplatform-prod`, NOT `rg-orbit-prod-eus2`), Container Apps Env `cae-orbit-prod-eus2`, Postgres `pg-orbit-prod`, App Config `appcs-orbit-prod-eus2`, App Insights `appi-oribit-prod` (sic), API managed identity `id-platformshared-prod`. Naming is inconsistent — several resources omit the `-eus2` suffix, the SWA uses `swa-` (not `stapp-`), and App Insights is misspelled. Shared/other resources (Front Door, Key Vault, ACR, Log Analytics, Storage, RG) were not in the portal listing and remain as specified above (unconfirmed).
...
- `AZURE_SUB_SHAREDPLATFORM` — subscription GUID for the shared-platform subscription (`sub-sharedplatform`) that hosts `ca-orbit-prod-v2`, `afd-shared-prod`, and shared VNets/Network Watchers. **Required for the network page to find any resources.** When set, every network Resource Graph query includes this subscription regardless of which app is in scope. Set it to the same GUID as `AZURE_SUB_ORBIT` if both live in `sub-sharedplatform`, or to a distinct GUID if they differ. Deduplicated automatically — no double-query if it matches a value already in `AZURE_SUBSCRIPTION_IDS`.

Required Azure RBAC for `id-platformshared-prod` on each subscription in `AZURE_SUBSCRIPTION_IDS` **and on the `AZURE_SUB_SHAREDPLATFORM` subscription** (sub-sharedplatform-prod): **Cost Management Reader** (Cost Management queries) + **Reader** (Resource Graph queries). The shared-platform sub hosts Container Apps, Front Door, and Network Watchers — without `Reader` on it, network-page Resource Graph queries will throw `AuthorizationFailed`. Assign via Azure Portal → Subscriptions → IAM → Add role assignment → Members tab → select Managed identity → `id-platformshared-prod`.

Also ensure `id-platformshared-prod` is **assigned to the Container App**: Azure Portal → Container Apps → ca-orbit-prod-v2 → Identity → User assigned → Add → select `id-platformshared-prod`.

**Live time-series charts — config-gated:** The telemetry and infrastructure routes query real hourly time-series from Log Analytics when `AZURE_LOG_ANALYTICS_WORKSPACE_ID` is set (the workspace **customer ID / GUID**, shown as "Workspace ID" in the Azure portal — not the resource ID). Set this on the Container App alongside the base Azure vars. When unset (or when any live query fails / returns empty data), the routes fall back to deterministic seeded mock series — Replit dev preview is unaffected. Managed identity `id-platformshared-prod` needs **Log Analytics Reader** on the workspace to run the KQL queries.

**Budget lookup — MCA billing scope (config-gated):** Azure Consumption SDK `budgets.list()` only works at subscription scope; MCA tenants create budgets at billing account level. When `AZURE_BILLING_ACCOUNT_ID` is set, a billing-account-scope lookup fires as a final fallback after subscription-scope strategies. Set `AZURE_BILLING_ACCOUNT_ID` to the MCA billing account ID (e.g. `ce5d8906-8bbf-4b89-9546-77b76f42d0f7`). Managed identity `id-platformshared-prod` needs **Billing Account Reader** on the billing account (Azure Portal → Cost Management + Billing → Billing account → Access control (IAM)).

**Deployments surface — requires `ORBIT_DEPLOYMENT_ID` GitHub secret (OrbitProduction env):** The deployments page fetches GitHub Actions run history via the GitHub API. The token must be a fine-grained PAT with Actions: Read-only on the tracked repos (`Orbit-Admin-Ctr`, `GrailBabe`). Named `ORBIT_DEPLOYMENT_ID` in the OrbitProduction environment secrets to avoid collision with the built-in `GITHUB_TOKEN` that Actions uses for GHCR login. The deploy workflow passes it to the Container App as the env var `GITHUB_TOKEN`.
...
- **Deploy workflow preserves all CA env vars including secret references.** The deploy script reads existing CA env vars before calling `az containerapp update --set-env-vars`, merging Portal-set values with GitHub secrets so nothing is wiped. This includes env vars stored as Azure CA secrets (`secretRef` entries) — previously those were silently dropped on every deploy. If you add a new env var to the CA via the Portal, it will survive future deploys automatically (no code change needed). If you want a var managed via GitHub instead, add it to the `OrbitProduction` environment secrets and the `secret_map` in `deploy-api-v2.yml`.

## Deferred / future work

- **Budget alert external channels — Teams and/or email notifications.** The in-app badge and alert history are live. To also push notifications outside Orbit when a budget overrun is detected: (1) **Teams** — create an Incoming Webhook in the target channel (Teams → Manage channel → Connectors → Incoming Webhook), then set `ALERT_TEAMS_WEBHOOK_URL` on `ca-orbit-prod-v2`; per-app overrides via `ALERT_TEAMS_WEBHOOK_URL__<APPID>`; (2) **Email (SMTP)** — set `ALERT_SMTP_HOST`, `ALERT_SMTP_PORT`, `ALERT_SMTP_USER`, `ALERT_SMTP_PASS`, `ALERT_SMTP_FROM`, `ALERT_EMAIL_TO` on the Container App; per-app recipient overrides via `ALERT_EMAIL_TO__<APPID>`. Both channels are independent and optional — neither is required for in-app alerts to work.
- **Apple App Store live store ingestion — next up (GrailBabe launches on iOS first).** Credentials already provisioned on `ca-orbit-prod-v2` (`APPLE_CONNECT_ISSUER_ID`, `APPLE_CONNECT_KEY_ID`, `APPLE_CONNECT_PRIVATE_KEY`, `APPLE_VENDOR_NUMBER`). Remaining work: implement `fetchLiveAppleSubscriptions()` in `artifacts/api-server/src/lib/appleSubscriptions.ts` to pull the `SUBSCRIPTION_EVENT` monthly report from App Store Connect, and implement the ledger sync route at `POST /api/apps/:appId/ledger/app-store/sync`. Until then syncing returns 503 and the subscriptions surface shows placeholder data.
- **Google Play live store ingestion — deferred until Android launch (after iOS).** Three things required when the time comes: (1) set up Azure → Google Workload Identity Federation trust (no downloadable JSON key per org policy); (2) add env vars to `ca-orbit-prod-v2`: `GOOGLE_PLAY_SA_EMAIL`, `GOOGLE_PLAY_WIF_AUDIENCE`, `GOOGLE_PLAY_REPORTING_BUCKET`, `GOOGLE_PLAY_DEVELOPER_ID`; (3) implement `fetchLivePlaySubscriptions()` in `artifacts/api-server/src/lib/playSubscriptions.ts` to pull the earnings CSV from the Play Console GCS bucket. Until then the Play surfaces serve deterministic placeholder data.
