...
export const ListAppsResponseItem = zod.object({
  ...
  "tags": zod.object({
  "Application": zod.string().optional().describe('Application tag value (e.g. \"GrailBabe\", \"Orbit\").'),
  "CostCategory": zod.string().optional().describe('Cost category tag value (e.g. \"WebApp\", \"Infrastructure\").'),
  "CostCenter": zod.string().optional().describe('Cost center tag value (e.g. \"Orbit\", \"GrailBabe\").'),
  "ServiceType": zod.string().optional().describe('Service type tag value (e.g. \"Web\", \"Database\").'),
...
}).describe('Azure resource tags applied to this application\'s resource group. The standard Kinisis tag keys are typed explicitly; any additional tags on the resource group are preserved as free-form string values.'),
...
export const GetAppResponse = zod.object({
  ...
  "playAppId": zod.string().optional().describe('Numeric Google Play app ID. When present, used to construct a direct link to the app\'s management page in Google Play Console.'),
...
export const ListPlaySubscriptionsResponseItem = zod.object({
  ...
  "playAppId": zod.string().optional().describe('Numeric Google Play app ID. When present, used together with playDeveloperId to construct a direct link to the app in Google Play Console.'),
  "playDeveloperId": zod.string().optional().describe('Google Play developer account ID. When present alongside playAppId, enables construction of a direct link into Google Play Console.'),
...
export const ListDeploymentsResponse = zod.object({
  ...
  "runUrl": zod.string().optional().describe('Direct link to this run on GitHub Actions.')
})),
...
export const ListAlertConfigResponseItem = zod.object({
  ...
  "cpuSource": zod.enum(['db', 'env', 'inventory', 'default']).optional().describe('Which source provides the effective CPU threshold: db = operator-saved DB override, env = per-app env var (ALERT_CPU_THRESHOLD_PCT__<APPID>), inventory = app built-in baseline from the APPS record, default = global env var or hardcoded 80%'),
  "memorySource": zod.enum(['db', 'env', 'inventory', 'default']).optional().describe('Which source provides the effective memory threshold: db = operator-saved DB override, env = per-app env var (ALERT_MEMORY_THRESHOLD_PCT__<APPID>), inventory = app built-in baseline from the APPS record, default = global env var or hardcoded 85%'),
  ...
  "cooldownSource": zod.enum(['db', 'env', 'appconfig', 'default']).optional().describe('Which source provides the effective cooldown (db = Orbit UI override, env = ALERT_COOLDOWN_HOURS[__APPID], appconfig = Azure App Configuration key ALERT_COOLDOWN_HOURS, default = 12h built-in)'),
