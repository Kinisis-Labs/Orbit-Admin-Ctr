...
async function checkGitHubToken(): Promise<CheckResult> {
  const token = process.env.GITHUB_TOKEN ?? process.env.ORBIT_DEPLOYMENT_ID;
  if (!token) return { status: "not_configured", detail: "GITHUB_TOKEN / ORBIT_DEPLOYMENT_ID not set" };
  try {
...
router.get("/diagnostics", requireAdmin, async (_req, res) => {
  ...
  const report = {
    ...
    env: {
      ...
      GITHUB_TOKEN: (process.env.GITHUB_TOKEN ?? process.env.ORBIT_DEPLOYMENT_ID)
        ? `✓ set via ${process.env.GITHUB_TOKEN ? "GITHUB_TOKEN" : "ORBIT_DEPLOYMENT_ID"} (${(process.env.GITHUB_TOKEN ?? process.env.ORBIT_DEPLOYMENT_ID)!.length} chars)`
        : "❌ not set (checked GITHUB_TOKEN + ORBIT_DEPLOYMENT_ID)",
