...
import {
  DollarSign, HeartPulse, Network, Tag,
...
function SectionHeader({
  ...
  return (
    <div className="p-2 border-b border-border flex items-center gap-2">
      ...
      {sub && <span className="text-[11px] text-muted-foreground ml-1 hidden sm:inline">{sub}</span>}
      <div className="ml-auto flex items-center gap-2">
        {right}
        {href && (
          <Link href={href} className="inline-flex items-center gap-0.5 text-[11px] text-primary hover:underline shrink-0">
...
function CostSection({ canSeeCost }: { canSeeCost: boolean }) {
  ...
  const totalCategorised = categorySpend.filter((c) => c.name !== "Untagged").reduce((s, c) => s + c.value, 0);
  const untaggedAmt = categorySpend.find((c) => c.name === "Untagged")?.value ?? 0;

  if (!canSeeCost) {
    return (
      <div className="bg-card border border-border shadow-sm">
        <SectionHeader icon={<DollarSign className="h-3.5 w-3.5" />} title="Cost by CostCategory" href="/cost" />
  ...
  return (
    <div className="bg-card border border-border shadow-sm">
      ...
      {isLoading ? (
        ...
        <div className="p-5 text-center space-y-1">
          ...
          <p className="text-[12px] text-muted-foreground">
            No <span className="font-mono">CostCategory</span> tags found on apps yet.{" "}
            Apply tags in Azure portal or check the <Link href="/tags" className="text-primary hover:underline">Tags page</Link>.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-border">
          {/* Bar chart */}
          <div className="p-4">
            <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">Spend breakdown</div>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={categorySpend} layout="vertical" margin={{ left: 0, right: 40, top: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="value" radius={[0, 2, 2, 0]}>
                    {categorySpend.map((entry) => (
                      <Cell key={entry.name} fill={COST_CATEGORY_COLOR[entry.name] ?? "#94a3b8"} fillOpacity={0.85} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          {/* Summary list */}
          <div className="p-4">
            <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">Category summary</div>
            <div className="space-y-2">
              {categorySpend.map((entry) => {
                ...
                return (
                  <div key={entry.name} className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] tabular-nums text-muted-foreground w-16 text-right">{fmtK(entry.value)}</span>
              ...
              {untaggedAmt > 0 && (
                <div className="pt-1 mt-1 border-t border-border/50 text-[11px] text-amber-500 flex items-center gap-1.5">
                  ...
                  {fmtK(untaggedAmt)} spend ({Math.round((untaggedAmt / totalMtd) * 100)}%) is missing a CostCategory tag
                </div>
              )}
      ...
      {/* Refresh indicator */}
      {isFetching && <div className="px-4 py-1 border-t border-border/40 text-[10px] text-muted-foreground">Refreshing…</div>}
    </div>
  );
}
...
function HealthSection() {
  ...
  const epUnhealthy = useMemo(() => (eps as GlobalEndpointRow[]).filter((e) => e.status === "unhealthy").length, [eps]);
  const epDegraded = useMemo(() => (eps as GlobalEndpointRow[]).filter((e) => e.status === "degraded").length, [eps]);
  const epHealthy = useMemo(() => (eps as GlobalEndpointRow[]).filter((e) => e.status === "healthy").length, [eps]);
  const avgLatency = useMemo(() =>
    eps.length ? Math.round((eps as GlobalEndpointRow[]).reduce((s: number, e: GlobalEndpointRow) => s + e.latencyMs, 0) / eps.length) : 0,
    [eps],
  );

  const sloBreachingErr = useMemo(() => (slos as SloRow[]).filter((s) => s.errorRatePct > s.errorTargetPct).length, [slos]);
  const sloBreachingLat = useMemo(() => (slos as SloRow[]).filter((s) => s.p95LatencyMs > s.p95TargetMs).length, [slos]);
  const sloMeetingUptime = useMemo(() => (slos as SloRow[]).filter((s) => s.uptimePct >= 99.9).length, [slos]);
  ...
  const worstEps = useMemo(() =>
    [...(eps as GlobalEndpointRow[])]
      ...
      .sort((a, b) => (STATUS_RANK[a.status] ?? 3) - (STATUS_RANK[b.status] ?? 3) || b.latencyMs - a.latencyMs)
      .slice(0, 4),
    [eps],
  );
  ...
  const sloTone = sloBreachingErr > 0 ? "bad" : sloBreachingLat > 0 ? "warn" : slos.length > 0 ? "ok" : "muted";
    ...
    <div className="bg-card border border-border shadow-sm">
      <SectionHeader
        ...
        right={
          epData?.dataSource && epData.dataSource !== "none" ? (
            <DataSourceBadge dataSource={epData.dataSource} dataAsOf={epData.dataAsOf} label="Azure" />
          ) : undefined
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3">
        ...
        <Tile
          ...
          tone={sloMeetingUptime === slos.length && slos.length > 0 ? "ok" : slos.length === 0 ? "muted" : "warn"}
        />
        <Tile
          title="SLO breaches"
          ...
          <div className="flex-1 min-w-0">
            ...
            {worstEps.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {worstEps.map((e) => (
                  <span key={e.id} className="text-[10px] px-1.5 py-0.5 rounded-sm bg-destructive/15 border border-destructive/30 text-destructive font-mono">
                    {e.appName} · {e.name}
                  </span>
                ))}
                <Link href="/network" className="text-[10px] text-primary hover:underline flex items-center gap-0.5">
      ...
      {hasSloIssues && (
        <div className="mx-3 mb-3 bg-amber-500/10 border border-amber-500/30 rounded-sm px-3 py-2 flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-[12px] font-semibold text-amber-600 dark:text-amber-400">SLO breaches — </span>
            ...
            {worstSlos.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {worstSlos.map((s) => (
                  <span key={s.appId} className="text-[10px] px-1.5 py-0.5 rounded-sm bg-amber-500/15 border border-amber-500/30 text-amber-600 dark:text-amber-400 font-mono">
                    {s.appName}
                  </span>
                ))}
                <Link href="/health" className="text-[10px] text-primary hover:underline flex items-center gap-0.5">
                  View all <ArrowRight className="h-2.5 w-2.5" />
                </Link>
              </div>
            )}
      ...
      {!hasNetworkIssues && !hasSloIssues && !epLoading && !sloLoading && eps.length > 0 && slos.length > 0 && (
      ...
      {epFetching && <div className="px-4 py-1 border-t border-border/40 text-[10px] text-muted-foreground">Refreshing…</div>}
    </div>
  );
}
...
function TagComplianceSection() {
  ...
  const byTag = useMemo(() => {
    ...
    return COMPLIANCE_REQUIRED_TAGS.map((tag) => ({
      tag,
      missing: data.entries.filter((e: { missingTags: string[] }) => e.missingTags.includes(tag)).length,
    }));
  }, [data?.entries]);
  ...
  const untaggedApps = apps ? apps.filter((a) => !(a.tags as Record<string, string> | undefined)?.["CostCategory"]).length : 0;

  const pcTone =
    compliancePct === null ? "muted" :
    compliancePct === 100 ? "ok" :
    compliancePct >= 80 ? "warn" : "bad";
  ...
  return (
    <div className="bg-card border border-border shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-border">
        {/* Compliance score */}
        <div className="p-4">
          <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">Azure resource compliance</div>
          {isLoading ? (
            <Skeleton className="h-12 w-full" />
          ) : data?.dataSource === "unavailable" ? (
            <div className="text-[12px] text-muted-foreground italic">Azure not configured — set AZURE_SUBSCRIPTION_IDS to enable scanning.</div>
          ) : data?.dataSource === "error" ? (
            <div className="text-[12px] text-destructive">{data.errorMessage ?? "Tag scan failed."}</div>
          ) : (
            <div className="space-y-3">
              <div className="flex items-baseline gap-2">
                <span className={`text-3xl font-bold tabular-nums ${pcTone === "bad" ? "text-destructive" : pcTone === "warn" ? "text-amber-500" : pcTone === "ok" ? "text-green-500" : "text-muted-foreground"}`}>
                  {compliancePct}%
                </span>
                <span className="text-[12px] text-muted-foreground">compliant</span>
                <StatusPill tone={pcTone}>
                  {pcTone === "ok" ? "All tagged" : pcTone === "warn" ? "Partial" : pcTone === "bad" ? "Gaps" : "—"}
                </StatusPill>
              </div>
              ...
              <div className="text-[11px] text-muted-foreground">
                {(data?.totalScanned ?? 0) - (data?.nonCompliantCount ?? 0)} of {data?.totalScanned ?? 0} resources fully tagged
              </div>
              {/* Per-tag breakdown */}
                  ...
                  <div key={tag} className="flex items-center gap-2 text-[11px]">
                    ...
                    <div className="flex-1 h-1 bg-muted rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: data?.totalScanned ? `${Math.round((missing / data.totalScanned) * 100)}%` : "0%" }}
                      />
                    </div>
                    {missing === 0 ? (
                      <span className="text-green-500 w-8 text-right">✓</span>
                    ) : (
                      <span className="text-destructive w-8 text-right tabular-nums">{missing}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        {/* CostCategory rollup */}
        <div className="p-4">
          <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">CostCategory coverage (apps)</div>
          {!apps ? (
            <div className="space-y-2">
              <Skeleton className="h-5 w-full" />
              <Skeleton className="h-5 w-4/5" />
            ...
            <div className="space-y-2">
              ...
              {untaggedApps > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-muted-foreground/60 italic px-1.5 py-0.5 border border-dashed border-border rounded-sm w-28 shrink-0">untagged</span>
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-destructive/40 transition-all" style={{ width: `${Math.round((untaggedApps / apps.length) * 100)}%` }} />
                  </div>
        ...
        {/* Quick actions */}
        <div className="p-4">
          <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">Quick links</div>
          <div className="space-y-1.5">
            {[
              { href: "/tags", label: "Tag inventory & compliance", icon: <Tag className="h-3.5 w-3.5" /> },
              { href: "/network", label: "Network health", icon: <Network className="h-3.5 w-3.5" /> },
              { href: "/health", label: "SLOs & error budgets", icon: <HeartPulse className="h-3.5 w-3.5" /> },
              { href: "/cost", label: "Cost Management", icon: <DollarSign className="h-3.5 w-3.5" /> },
            ].map(({ href, label, icon }) => (
              <Link
                key={href}
                href={href}
