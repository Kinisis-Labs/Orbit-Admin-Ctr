...
// --- Users & activity (corporate Entra-shaped) ---------------------------
...
export function buildUserActivity(apps: AppLite[]): UserActivityRow[] {
  return apps.map((app) => {
    const rand = seededRand(app.id + "users");
    const tierBase = app.environment === "prod" ? 4000 : app.environment === "staging" ? 200 : 30;
    const totalMembers = Math.floor(tierBase * (0.6 + rand() * 1.4));
    const mau = Math.floor(totalMembers * (0.5 + rand() * 0.4));
    const wau = Math.floor(mau * (0.35 + rand() * 0.3));
    const dau = Math.floor(wau * (0.25 + rand() * 0.3));
    return {
      appId: app.id,
      appName: app.name,
      environment: app.environment,
      ...
      wau,
      mau,
      inactive30d: totalMembers - mau,
      newLast7d: Math.floor(totalMembers * (0.01 + rand() * 0.04)),
      dauTrendPct: Number((rand() * 30 - 12).toFixed(1)),
    };
  });
}
...
const FIRST = ["Aria", "Ben", "Cleo", "Devon", "Esha", "Finn", "Gia", "Hugo", "Ivy", "Jonas", "Kira", "Luca", "Maya", "Niko", "Omar", "Pia", "Quinn", "Rosa", "Sami", "Tara"];
const LAST = ["Allen", "Brooks", "Chen", "Diaz", "Ellis", "Foster", "Gomez", "Huang", "Imani", "Jung", "Kapoor", "Liu", "Mehta", "Novak", "Oduya", "Patel", "Reyes", "Singh", "Tran", "Vega"];

export function buildUserSample(apps: AppLite[], perApp = 8): UserRecord[] {
  const out: UserRecord[] = [];
  for (const app of apps) {
    const rand = seededRand(app.id + "user-sample");
    for (let i = 0; i < perApp; i++) {
      const fn = pick(rand, FIRST);
      const ln = pick(rand, LAST);
      const minsAgoSignIn = Math.floor(rand() * 60 * 24 * 90);
      const minsAgoActive = Math.max(0, minsAgoSignIn - Math.floor(rand() * 120));
      const minsAgoCreated = minsAgoSignIn + Math.floor(rand() * 60 * 24 * 365);
      const lastActive = new Date(Date.now() - minsAgoActive * 60 * 1000);
      const daysSinceActive = minsAgoActive / 60 / 24;
      const state: UserRecord["state"] =
        daysSinceActive <= 1 ? "Active" : daysSinceActive <= 30 ? "Idle" : "Inactive";
      out.push({
        id: `${app.id}-user-${i}`,
        ...
        appId: app.id,
        appName: app.name,
        lastSignInAt: new Date(Date.now() - minsAgoSignIn * 60 * 1000).toISOString(),
        lastActiveAt: lastActive.toISOString(),
        createdAt: new Date(Date.now() - minsAgoCreated * 60 * 1000).toISOString(),
        state,
      });
    }
  }
